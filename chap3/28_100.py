...
# 条件
# [[(hoge#hoge|)表示文字]]
# [[()]]は削除対象。()はあるかわからない。表示文字だけ残す。
info[0] = re.sub(r"\[\[([^|#\]]+?\|)*(.*?)\]\]",r"\2",str(info[0]))
...
info[0] = re.sub(r"<\/?[br|ref][^>]+?\/?>","",str(info[0]))
...
info[0] = re.sub(r"\[http:[^\]]+?\]","",str(info[0]))
...
info[0] = re.sub(r"\{\{lang\|[^\|]+?\|([^\}]+?)\}\}",r"\1",str(info[0]))
